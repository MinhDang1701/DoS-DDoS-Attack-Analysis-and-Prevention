...
This document provides a quick mapping between the `.docx` report structure and the repository files, facilitating the evaluator's review.

**Topic:** Denial of Service (DoS/DDoS) Attack Analysis and Prevention
**Student:** Minh Dang — ID: 30211456495
...
4.2 Principles of DoS/DDoS Detection and Mitigation (threshold‑based / signature‑based / anomaly‑based, IDS/IPS)
4.3 Related Standards (RFC 4987, RFC 4732, NIST SP 800‑189, CISA)
4.4 Application of AI/ML to DDoS Detection and Mitigation (future directions)
...
## Data Mapping Notes (Table 5.2 in the report)

| Scenario   | Baseline | Peak during attack | Defense outcome |
|------------|----------|--------------------|-----------------|
| SYN Flood  | ~0–2 pkt/s | ~12,500–14,000 pkt/s (~115 s) | TCP rule verified — DROP SYN logged |
| UDP Flood  | ~0–2 pkt/s | ~20,500 pkt/s (at 3 s) | UDP rule configured & enabled (no DROP log because the lab does not host a UDP service) |
